...
$usage .= "Options:\n";
$usage .= "      --suppressZeros  do not print out result rows with zero count.\n";
$usage .= "      --outfiles       If this flag is set, combination sets are created.\n";
$usage .= "      --snsp           Comput sensitivity and specificity. Requires n=2,\n";
$usage .= "                       simple mode and assumes that the first set is the truth.\n";
...
my $suppressZeros = 0; # if true, do not print out result rows with zero count
my $snsp = 0;
...
if ($snsp && $n != 2){
    print STDERR "With option --snsp there must be exactly 2 input files\n";
    print $usage;
    exit;
}
...
print " | # of elements\n----------------------------------\n";
my ($TP,$FP,$FN) = (0,0,0); # true/false positives/negatives for the case in which sensitivity specificy is computed
for (my $j=1; $j < (2**$n); $j++) {
	...
	for (my $i=0; $i<$n && $is_element; $i++){
	    my $in_set = (defined($sets[$i]->{$item}) && $sets[$i]->{$item}>0);
	    if (! ($in_set == $elem[$i])){
		$is_element = 0;
	    }
	}
    ...
    next if ($k == 0 && $suppressZeros);
    print "combination $combstr | $k\n";
    $combstr =~ s/ //g;
    if ($outfiles) {
...
if ($snsp && $TP+$FN>0 && $TP+$FP>0){
    my $sensitivity = $TP/($TP + $FN);
    my $specificity = $TP/($TP + $FP);
    print "TP=$TP\tFP=$FP\tFN=$FN\n";
    print "sensitivity TP/(TP + FN) = $sensitivity\n";
    print "specificity TP/(TP + FP) = $specificity\n";
}
